...
export const LightingSection: React.FC = () => {
  ...
  return (
    <section id="lighting" className="py-20 sm:py-28 bg-[#140204] text-[#FFFDD0] relative overflow-hidden">
      {/* Ambient Lighting Glow Effects */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-full max-w-4xl h-72 bg-[#D4AF37]/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="max-w-3xl mx-auto text-center mb-14 text-[#FFFDD0]/80 text-sm sm:text-base leading-relaxed">
          <p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {lightingGrid.map((item, index) => (
            <div
              key={index}
              className="group relative rounded-sm bg-[#5D0E11] border border-[#D4AF37]/40 hover:border-[#D4AF37] overflow-hidden shadow-xl transition-all duration-300 hover:-translate-y-1.5 flex flex-col justify-between"
            >
              <div className="relative aspect-[4/3] w-full overflow-hidden bg-[#2D080E]">
                <img
                  src={item.image}
                  alt={item.title}
                ...
                <span className="absolute top-3 left-3 px-2.5 py-0.5 rounded-sm text-[10px] font-bold tracking-widest uppercase bg-[#140204]/90 text-[#D4AF37] border border-[#D4AF37]/40">
                  {item.tag}
                </span>

                <span className="absolute bottom-2 right-3 text-[10px] text-[#FFFDD0]/60 italic">
                  Lighting Asset
                </span>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  <h4 className="font-cinzel text-base sm:text-lg font-bold text-[#FFFDD0] group-hover:text-[#D4AF37] transition-colors mb-2">
                    {item.title}
                  </h4>
                  <p className="font-sans text-xs sm:text-sm text-[#FFFDD0]/80 leading-relaxed">
                    {item.desc}
                  </p>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-[#D4AF37]/20 flex items-center justify-between text-xs text-[#D4AF37]">
                  ...
                  <span className="text-[#FFFDD0]/60 font-bold">0{index + 1}</span>
                </div>
              </div>
            </div>
          ))}
